...
import com.libraryclient.utils.*;
import java.io.*;
...
public class DeliciousAccount implements Authentication
{
	...
	private String encryptPassword(String password)
	{
		try
		{
			byte[] output = crypto.encryptAsBase64(password.getBytes(HTTP.UTF_8));
	...
	private String getPassword()
	{
		...
		try
		{
			byte[] output = crypto.decryptAsBase64(encrypted.getBytes(HTTP.UTF_8));
			return new String(output, HTTP.UTF_8);
		}
		catch (UnsupportedEncodingException e)
		{
